...
def meaningful_lines(value):
    if isinstance(value, list):
        return [str((item.get("item") or item.get("text") or item.get("original") or "") if isinstance(item, dict) else item).strip()
                for item in value if str(item).strip()]
    return [line.strip() for line in str(value or "").splitlines() if line.strip()]


def validate_recovery_patch(current, patch):
    if not isinstance(patch, dict) or not patch or set(patch) - RECOVERY_FIELDS:
        raise ValueError("Recovery patch contains unsupported fields")
    for field in patch:
        if field == "recoveryProvenance":
            continue
        if current.get(field) not in (None, "", [], {}):
            raise ValueError(f"Field already contains data: {field}")
    merged = safe_recipe_merge(current, patch)
    merged["name"] = current.get("name")
    merged["link"] = current.get("link")
    ingredients = max((meaningful_lines(merged.get(key)) for key in ("structuredIngredients", "ingredientsText")), key=len)
    instructions = max((meaningful_lines(merged.get(key)) for key in ("structuredInstructions", "instructions")), key=len)
    joined = " ".join(ingredients + instructions).lower()
    if not ingredients or not instructions or "[object object]" in joined or joined.strip() in ("ingredients instructions", "ingredienser se video"):
        raise ValueError("Patch does not create meaningful complete recipe content")
    return merged
